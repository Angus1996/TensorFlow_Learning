...
def variable_summaries(var, name):
    ...
    with tf.name_scope('summaries'):
        ...
        # 名为‘mean/’+name, 其中mean为命名空间，/是命名空间的分隔符。
...
def main():
    ...
    # 给定的数据为验证或者测试数据，那么得到的正确率就是在当前模型在验证或者测试数据上的正确率
    ...
    # 和TensorFlow中其他操作类似，tf.scalar_summary、tf.histogram_summary
    # 和tf.image_summary函数都不会立即执行，需要通过sess.run来明确调用这些函数。
    # 因为程序中定义的写日志操作比较多，一一调用非常麻烦，所以TensorFlow程序执行
    # 的过程中只需要运行这个操作就可以将代码中定义的所有日志生成操作执行一次从而将所有日志写入文件
